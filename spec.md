...
- Server-side only: the `kidstory-api` key lives in `runtimeConfig` (never sent to the client); one Nitro server route asks the API to draft the story (`POST /stories/generate`, which runs Kidstory's own fine-tuned model) and another asks it to store the accepted draft (`POST /stories`).
...
- A new story is generated and published unattended once a day, every morning (Vercel Cron, `0 2 * * *` = ~07:00 Asia/Karachi — the Hobby plan fires it sometime within the 02:00 UTC hour), on top of (not replacing) manual `/studio` use — see §8 for the mechanism.
- Topics are drawn from a curated seed pool (`shared/utils/topic-pool.ts`, ~60 premises spread across the six categories) without repeats, tracked via a Supabase `used_topics` table; once the pool is exhausted the model is asked to invent a fresh topic distinct from everything used so far.
- Same as `/studio`, a successful automated post also emails the owner a copy of the new story and posts to the Facebook Page and Instagram. The daily Vercel run does **not** post to YouTube (video rendering needs `ffmpeg-static`, which exceeds Vercel's serverless limits) — YouTube auto-post runs from `/studio` publish and the manual GitHub Actions fallback only.
...
- **AI story authoring:** `/studio`, the Vercel cron route and `scripts/daily-post.ts` all draft stories through `kidstory-api`'s `POST /stories/generate` via the shared `generateStoryDraft()` (`shared/utils/story-authoring.ts`). The API runs **Kidstory's own model**, `ziaflutter/tinyllama-cat-mouse-story` (a TinyLlama-1.1B fine-tune trained by the owner in Colab), served from a free Hugging Face Docker Space with llama.cpp and called via `huggingface_hub`'s `InferenceClient` — see `../kidstory-api/README.md` and `deploy/model-space/`. It is a prose model, not a JSON one: the API derives title/excerpt/emoji/tags/read-time from the text. No third-party LLM key is needed anywhere in this app any more (OpenRouter, and Groq before it, are gone). The Nuxt side only needs `KIDSTORY_API_URL`/`KIDSTORY_API_KEY`. Publishing is `POST /stories` on the same API.
...
- **Automated publishing:** primary scheduling is **Vercel Cron** — `vercel.json`'s `crons` entry hits `server/api/cron/daily-post.get.ts` on the deployment once a day (`0 2 * * *`). That route picks an unused topic (`shared/utils/auto-post.ts`'s `pickAutoPostTopic()`, backed by the Supabase `used_topics` table), generates and publishes via the same `generateStoryDraft()`/`publishStoryDraft()` `/studio` uses (`shared/utils/story-authoring.ts`), records the topic right after publish (so a timeout can't cause a repeat), then emails the owner and posts to Facebook + Instagram. It's guarded by a `CRON_SECRET` bearer check (Vercel Cron sends `Authorization: Bearer ${CRON_SECRET}`). Needs `CRON_SECRET` plus the `OPENROUTER_API_KEY`/`SUPABASE_*`/`GMAIL_*`/`NOTIFY_EMAILS`/`SITE_URL`/`FACEBOOK_*`/`INSTAGRAM_*` values as Vercel env vars. `nuxt.config.ts` sets `nitro.vercel.functions.maxDuration = 60` (the Hobby-plan ceiling) since the LLM call plus posts exceed the default 10s. `scripts/daily-post.ts` is the same pipeline as a standalone script (`npx tsx` / `npm run auto-post`), kept as a manual fallback — it additionally posts to YouTube, and is what the now manual-only `.github/workflows/auto-post.yml` (`workflow_dispatch` only) runs. GitHub Actions was the original scheduler, chosen when the app had no deployment; Vercel Cron replaced it once the app was live on Vercel (see decision log).
